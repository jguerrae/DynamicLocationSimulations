...
*** = 4,439,412,000 OBSERVATIONS ------> NO ENTIENDO EL 4572.
***
*** STEP II. COLLAPSE RANDOM REALIZED FIRM-COUNTY-YEAR OUTCOMES TO REALIZED COUNTY-YEAR OUTCOMES -> MERGE RANDOM REALIZED COUNTY-YEAR OUTCOMES DATASET TO DATASET OF SKELETON OF ALL POSSIBLE COUNTY-YEAR OUTCOMES AND THEN FILL REST WITH 0s (1,925 COUNTIES X 36 YEARS X 1,000 REPLICATIONS = 69.3 MILLION OBSERVATIONS) --->OK
***
*** STEP I. CREATE FIRM-COUNTY-YEAR RANDOM REALIZED OUTCOMES DATASET
***
*** 1. CREATE DATASET SKELETON = 971 FIRMS (ASSEMBLERS THAT ENTERED DURING PLC 1895-1930) X 1,000 REPLICATIONS = 971,000 OBSERVATIONS -->OK
*** 2. ALLOCATE TO EACH OF THE 971,000 FIRMS PSEUDO-RANDOMLY A PAIR OF MATCHED ENTRY YEAR AND SMITHCOMAPANYCODE ASSEMBLER ID -->OK
*** 3. ALLOCATE TO EACH OF THE 971,000 FIRMS PSEUDO-RANDONLY A LOCATION WITHIN THE SET OF 1,925 POSSIBLE COUNTIES, FOLLOWING THE 1890 CROSS COUNTY SHARE OF MANUFACTURING VARIABLES (ESTABLISHMENTS, INCOME MARKETS ACCESS, INCOME FOREIGN MARKET ACCESS, OR KNOWLEDGE STOCK) --> OK, EN ESTE SENTIDO ES DETERMINISTICO
*** 4. ALLOCATE TO EACH OF THE 971,000 FIRMS PSEUDO-RANDOMLY A SURVIVAL AGE, WITH SURVIVAL ONE MORE YEAR=1 IF RANDOM SHOCK IS LOWER THAN ESTIMATED PROBABILITY OF SURVIVAL ONE MORE YEAR, GIVEN ASSEMBLER AGE ---> OK
***
*** STEP II. CREATE FULL DATASET
*** 
*** 1. COLLAPSE FIRM-COUNTY-YEAR RANDOM REALIZED OUTCOMES DATASET INTO COUNTY-YEAR RANDOM REALIZED OUTCOMES DATASET --> OK
*** 2. MERGE REALIZED RANDOM COUNTY-YEAR OUTCOMES DATASET WITH DATASET OF ALL POSSIBLE COUNTY-YEAR OUTCOMES --> OK
***
****************************************************************
****************************************************************
****************************************************************
...
cd "D:\CLOUD\Universidad de los Andes\Xavier Hernando Duran Amorocho - Jorge Guerra\MCs\MC_v3 - repaso Jorge (feb 2022)\nestab90\"

use "MC_inputdata_year_entry.dta" // EN CADA AÑOS CUÁNTOS ENSAMBLADORES ENTRARON // 

expand entry // EXPANDO NUMERO DE ENSAMBLADORES QUE ENTRARON CADA AÑO, ESTO ME DEJA CON 971 OBSERVACIONES, UNA PARA CADA FIRMA // 
gen firmid= _n // LE ASIGNO UN ID A CADA FIRMA QUE ENTRO CADA AÑO (971) // 
order firmid
gen rep=1000 
expand rep // GENERO MIL ESCENARIOS DISTINTOS POR LA CANTIDAD DE FIRMAS QUE ENTRO CADA AÑO, IE, EN 1895 --> 4MIL, ESTO IMPLICA QUE EL DATASET QUEDA DE 971MIL // 
bysort firmid: gen repid= _n // PARA CADA FIRMA LE ASIGNO UN ID UNICO DE REPLICACIÓN // 
...
set obs 971000
set seed 4540
gen smithrandom = floor((970+1)*runiform() + 1) // GENERO UN CODIGO UNICO ENTRE 1 Y 971 PARA CADA POSIBLE OUTPUT //
bysort repid: egen smithid=rank(smithrandom), unique // POR REPLICACIÓN, GENERO UN CODIGO UNICO ENTRE 1 Y 971 PARA CADA POSIBLE OUTPUT (FIRMA). EJ: LA FIRMID 1 APARECE 1000 VECES EN 1985. EL REPID VA DEL 1 AL 1000. DENTRO DE ESE REPID VA UN NUMERO DEL 1 AL 971, ie, EN 1895 HABÍAN 4000 ESCENARIOS PARA ENTRAR, EN 1000 DE ESOS REPARTIERON NUMEROS UNIFORMEMENTE ENTRE 1 Y 971, ALGUNAS SE REPITEN.

//EN EL PASO ANTERIOR SE LE DIÓ UNA IDENTIDAD DE FIRMA A CADA REALIZACIÓN. ESA ASIGNACIÓN ES IDENTIDAD ES UNIFORME//
...
set obs 971000
set seed 3219
gen cfipsfirmsid = floor(( 100012 +1)*runiform() + 1) // LE ASIGNO A CADA REALIZACIÓN (YA CON LA IDENTIDAD DE FIRMA), SU CORRESPONDIENTE COUNTY. EL 100012 COINCIDE CON LAS DIMENSIONES DEL DATASET DE PARTICIPACIÓN. ESTA ASIGNACIÓN SIGUE SIENDO UNIFORME.//

//AÚN NO HAY MANERA DE DECIR QUE NY TENDRÁ MÁS PARTICIPACIÓN QUE CUYAHOGA //
...
drop _merge // HAGO EL MERGE DE LA IDENTIDAD DE CADA FIRMA CON SU CORRESPONDENCIA OBSERVADA //

sort cfipsfirmsid
merge m:m cfipsfirmsid using "MC_inputdata_cfips_nestab90.dta" // HAGO EL MERGE DE CADA COUNTY CON SU CORRESPONDENCIA REAL. SI BIEN EL CFIPSFIRMSID SIGUE SIENDO UNIFORME EL CFIPS_FIRMS NO Y ESTE ES QUE CONSERVA EL SHARE DE COUNTYS EN 1890, EN ESTE PASO NY YA TIENE MÁS PARTICIPACIÓN QUE CUYAHOGA //
...
//DE MOMENTO TODO ES UNIFORME EXCEPTO: LA PRESENCIA DE LOS AÑOS - LA CANTIDAD DE FIRMAS CADA AÑO, SMITHCOMPANYCODE, CFIPS_FIRMS (COUNTYS) Y MC_NESTAB (PARTICIPACIÓN)//
//SIGO TENIENDO UNA FIRMA DETERMINADA DE LAS 4 QUE ENTRARON EN 1985, QUE PUDO ENTRAR 1000 VECES A UN COUNTY DISTINTO (CONERVANDO LAS PROPORCIONES DE APARICIÓN). ASÍ PARA CADA AÑO TENDRÉ 1000 * Q FIRMAS QE ENTRARON // 
//EL CAMBIO DE ESTATICO (DISTRIBUCIÓN EN EL 90) A DINAMICO DEBE HACERSE ACÁ,  ++++RECORDAR LA DISCUSIÓN DE SI ERA O NO SIN REEMPLAZAR++++ //

tab year 
tab year if cfips_firms == 36061

// AL MIRAR EL  TAB POR AÑO SE OBSERVA QUE NY SIEMPRE TIENE UNA PARTICIPACIÓN DEL 8% APROX, IE, LA APARICIÓN DE CADA COUNTY CONSERVA LA PROPORCIÓN DEL 90 //
// ++++BUEN EJEMPLO KEEP IF SMITHID == 400 ++++ //
...
gen maxage=1930-year+1 // EDAD MAXIMA QUE PUEDE VIVIR UNA FIRMA (EJ: SI ENTRÓ EN 1907 EN NUEVAYORK SOLA PODRÁ VIVIR 23 AÑOS)
expand maxage // EXPANDO EL DATASET Y QUEDA 21 MILLONES ( ESTO ES INCORPORANDO LOS AÑOS MÁXIMOS QUE VIVIRÁ CADA FIRMA )

***************************************

bysort repid firmid: gen yearid= _n //ORDENANDO POR REPLICACIÓN FIRMA LE ASIGNO UN ID PARA CADA AÑO QUE ESTÁ VIVA (1,2,3,4,....n)

//RESUMEN: (ESCENARIO MEDIO) DE 1000 MUNDOS POSIBLE POR LA CANTIDAD DE FIRMA QUE ENTRAN CADA AÑO, UNA FIRMA (1971) ENTRA CON PROBABILIDAD SHARE_COUNTY A UN COUNTY (1925) DADO Y PUEDE SOBREVIVIR 1930-ENTRY+1 AÑOS. SE SIMULA ESA EXISTENCIA

***************************************

rename year entryyear //REBAUTIZO AL AÑO DE ENTRADA
gen year=entryyear+yearid-1 // EL NUEVO AÑO ES EL AÑO EN EL QUE ESTÁ VIVA (EJ: ENTRÉ EN 1920, SIGUE VIVA EN EL 21 (YEAR ID = 2) Y EL YEAR NUEVO SERÍA 1921)
...
set obs 21205001
set seed 8631
gen psurvival2 = runiform() // GENERO UNA PROBABILIDAD DE SOBREVIVIR PARA CADA AÑO
gen s=0 // DUMMY DE SUPERVIVENCIA


replace s=1 if yearid==1 //EL PRIMER AÑO SOBREVIVEN TODAS
...
///ESTE LOOP ME DA LA SUPERVIVENCIA LA FIRMA EN CADA AÑO


 ///++++¿CÓMO INTERPRETO LOS 0'S Y 1'S SI SON INTERMITWNTES? / SI SE MUERE ¿CÓMO APARECERÍA NUEVAMENTE?++++ // 
...
replace survival2=1 if year==entryyear // EL EQUIVALENTE DE ESTA LINEA PODRÍA SER COMO LA ANTERIOR replace survival2=1 if yearid==1
replace survival2=1 if survival2[_n-1]==1 & s==1 & year>entryyear //SOBREVIVE SI LA PROBABILIDAD DE SOBREVIVIR EN PASO EL THRESHOLD Y EL AÑO PASADO TAMBIÉN SOBREVIVIÓ, DLC LA MIERA QUEDA MUERTA POR EL RESTO DE LAS REALIZACIONES
bysort repid firmid: egen agedraw2=sum(survival2) //CANTIDAD DE AÑOS QUE SOBREVIVE

************************************************************************************************************************************************************************************************

// +¿POR QUÉ ES SIN REEMPLAZO? VOLVER A DISCUTIR IMPLICACIONES DE ESTO CON XAVIER


*** VARIABLES HELPFUL TO CHECK THAT RANDOM DRAWS WITHOUT REPLACEMENT INDEED WORKED THAT WAY

bysort repid firmid: gen agecountsurvival2=_n if survival2==1 //ORDENANDO POR REPLICIÓN - FIRMA GENERAR AÑOS ACUMULADOS QUE LLEVA SOBREVIVIENDO

replace agecountsurvival2=0 if survival2==0 // 0 CON AÑOS QUE NO SOBREVIVIÓ
...
// set obs 1000
// gen repid = _n
// cross using MC_inputdata_year_entry
// cross using MC_inputdata_cfips
// drop entry
...
collapse (sum) survival2, by (repid year cfips_firms) // EN CADA REPLICACIÓN-AÑO SUMO EL NÚMERO DE FIRMAS QUE SOBREVIVEN EN CADA COUNTY ---> DATASET DE 9,5 MILLONES
...
///SI UN COUNTY TIENE NANS ES DEBIDO A QUE NO TIENE ACTIVIDAD
...
*** (3.3.2) GENERATE OPB2 = OBSERVED AS RANGE OF RANDOM PLC // ¿POR QUE NECESITAMOS QUE EL A>0?
...
forval i= 1910(20)1930 {
	...
	keep year a cfips survivalB2p90 plq opB2 cname
	keep if year == `i'
	rename year year_`i'
	rename a a_`i'
	rename opB2 opB2_`i'
...
*** (6) STEP 4 GENERATE copB2_1: OBSERVED AS PERCENTAGE OF RANDOM PLC /// ESTO ES OPB2 PERO DE MANERA CONTINUA
...
save "B\cfips_firms.dta", replace // IS THE SAME MC_outputdata_random_PLC_inc_all_possible_outcomes_rep_year_cfipsfirms.dta
...
// ssc install estout (ESTADISTICAS DE TODOS LOS AÑOS)
...
////////////////  MISMO PASO 6 PERO SIN LOS AÑOS DE INTERÉS, ES TODO CONTINUO. ALGUNA VES XAVIER ME LO PIDIÓ
