...
///JG: ¿Cuál es la explicación del paso anterior?
...
*** 2. ALLOCATE TO EACH OF THE 971,000 FIRMS PSEUDO-RANDOMLY A PAIR OF MATCHED ENTRY YEAR AND SMITHCOMAPANYCODE ASSEMBLER ID // -->OK, ¿Por qué un código Smith asembler id para cada 971mil y no completa?
*** 3A. ALLOCATE TO EACH OF THE 971,000 FIRMS PSEUDO-RANDOMLY A LOCATION WITHIN THE SET 1,925 POSSIBLE CFIPS COUNTY CODES (1,925 COUNTIES HAVE FULL INFO FOR ANALYSIS) // -->OK, county
*** 3B. ALLOCATE TO EACH OF THE 971,000 FIRMS PSEUDO-RANDONLY A LOCATION WITHIN THE SET OF 337,902 POSSIBLE FIRM-CFIPS COUNTY CODES (337,902 FIRM-COUNTY ) // -->OK, pero no entiendo este paso. De donde sale el 337,902? CREI QUE YA RECUERDO PERO NECESITO PREGUNTARLE NUEVAMENTE
*** 4A. ALLOCATE TO EACH OF THE 971,000 FIRMS PSEUDO-RANDOMLY A SURVIVAL AGE 1, SURVIVAL AGE DRAWN FROM THE DISTRIBUTION OF OBSERVED SURVIVAL AGES //  -->OK
*** 4B. ALLOCATE TO EACH OF THE 971,000 FIRMS PSEUDO-RANDOMLY A SURVIVAL AGE 2, SURVIVAL=1 IF RANDOM SHOCK IS LOWER THAN ESTIMATED PROBABILITY OF SURVIVAL ONE MORE YEAR, GIVEN ASSEMBLER AGE // ---> ¿No sería mayor que?
***
*** WE HAVE CREATED 4 SCENARIOS OF RANDOM PLC INDUSTRY EVOLUTION
*** SCENARIO 1 (A1): RANDOM ENTRY ACROSS LOCATIONS AND SURVIVAL AGE DRAWN FROM OBSERVED DISTRIBUTION AT BIRTH -->Nacimiento
*** SCENARIO 2 (A2): RANDOM ENTRY ACROSS LOCATIONS AND SURVIVAL AGE DRAWN FROM ESTIMATED PROBABILITY OF SURVIVAL ONE MORE YEAR GIVEN FIRM IS ALIVE AT TIME T // -->Sobrevivir un año más en cada momento del tiempo
*** SCENARIO 3 (B1): RANDOM ENTRY ACROSS LOCATIONS WEIGHTED BY MANUFACTURING BASE AND SURVIVAL AGE DRAWN FROM OBSERVED DISTRIBUTION AT BIRTH --> Nacimiento ponderado el peso del county? (manufacturing base)
*** SCENARIO 4 (B2): RANDOM ENTRY ACROSS LOCATIONS WEIGHTED BY MANUFACTURING BASE AND SURVIVAL AGE DRAWN FROM ESTIMATED PROBABILITY OF SURVIVAL ONE MORE YEAR GIVEN FIRM IS ALIVE AT TIME T ---> ponderado por el peso y con la probabilidad de dobrevivir cada año
***
*** SCENARIOS A1 AND B1 GIVE RANDOM PLC INDUSTRY EVOLUTION IS LOWER BOUND ESTIMATE OF RANDOM LOCATION FOR COUNTIES WITH LARGE MANUFACTURING BASE //pedirle que me explique estas dos líneas
...
*** 
*** MERGE REALIZED RANDOM OUTCOMES DATASET WITH DATASET OF ALL POSSIBLE OUTCOMES (971 X 1,925 X 36 X 1,000 = 67,290,300,000) --->OK
***
...
*** MC_inputdata_year_entry.dta = OBSERVED NUMBER OF ENTRANTS PER PLC YEAR, 1895-1930 /// CUANTAS FIRMAS ENTRARON CADA AÑO 

*** MC_inputdata_smith.dta = SMITHCOMPANYCODE OF 971 ASSEMBLERS /// CODIGOS DE LAS 971 FIRMAS (smithid (del 1 al 10) y smithcompanycode)

*** MC_inputdata_cfips.dta = COUNTY CFIPS OF 1,925 POSSIBLE LOCATIONS (WITH FULL INFO FOR ANALYSIS) /// EL ID DE CADA FIRMA (1 AL 10)

*** MC_inputdata_cfipsfirms.dta = COUNTY CFIPS OF 337,902 POSSIBLE FIRM-LOCATION (WITH FULL INFO FOR ANALYSIS) /// misma pregunta anterior (MIRAR DATASET Y PREGUNTARLE)
