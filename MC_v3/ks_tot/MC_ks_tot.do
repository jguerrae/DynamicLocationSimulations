...
cd "D:\CLOUD\Universidad de los Andes\Xavier Hernando Duran Amorocho - Jorge Guerra\MCs\MC_v3\ks_tot\"
...
set obs 971000
set seed 3219
gen cfipsfirmsid = floor(( 99979 +1)*runiform() + 1)
...
sort cfipsfirmsid
merge m:m cfipsfirmsid using "MC_inputdata_cfips_ks_tot.dta"
drop _merge
drop in 971001/971005
...
foreach x of numlist 1900 1910 1920 1930{
	...
	if year==`x', title(Kernel ks_tot `x' ) graphregion(color(white)) xscale(range(0 5))
...
twoway  (kdensity copB2_3 if year ==1900, lcolor(black)) ///
 ...
 title(TS Kernel ks_tot ) graphregion(color(white)) xscale(range(0 5)) ///
...
foreach x of numlist 36061 25025 42101 17031 39061 39035 26163 18141 17019 {
	...
	title(ks_tot_survivalB2 & observed: `x') graphregion(color(white))
